...
if($result->num_rows > 0) {
    ...
    $query = "INSERT INTO Frame (Frame_ID, Frame_Number, Player_ID, Roll_One_ID, Roll_Two_ID, Team_ID, Game_ID, Date_Added) values ($newFrameID, $frameNumber, $playerID, $newRollID, $newRollID2, $teamID, $gameID, NOW())";
    $result = $conn->query($query);
    if($result){
    }
    else{
...
if(isset($_POST['submit'])) {
    if(isset($_POST['ballID'])) {
        ...
        if(isset($_POST['isFoul'])){
            ...
            $isStrike = 1;
            $query = "INSERT INTO Roll (Roll_ID, Frame_ID, Ball_ID, Is_Strike, Is_Foul, Hit_Pin_1, Hit_Pin_2, Hit_Pin_3, Hit_Pin_4, Hit_Pin_5, Hit_Pin_6, Hit_Pin_7, Hit_Pin_8, Hit_Pin_9, Hit_Pin_10) values ($currentRoll, $frameID, $ballID, $isStrike, $isFoul, $pinHit[0],$pinHit[1],$pinHit[2],$pinHit[3],$pinHit[4],$pinHit[5],$pinHit[6],$pinHit[7],$pinHit[8],$pinHit[9])";
        } else {
            $query = "INSERT INTO Roll (Roll_ID, Frame_ID, Ball_ID, Is_Strike, Is_Foul, Hit_Pin_1, Hit_Pin_2, Hit_Pin_3, Hit_Pin_4, Hit_Pin_5, Hit_Pin_6, Hit_Pin_7, Hit_Pin_8, Hit_Pin_9, Hit_Pin_10) values ($currentRoll, $frameID, $ballID, $isStrike, $isFoul, $pinHit[0],$pinHit[1],$pinHit[2],$pinHit[3],$pinHit[4],$pinHit[5],$pinHit[6],$pinHit[7],$pinHit[8],$pinHit[9])";
        }
        $result = $conn->query($query);
...
function calculateNumberOfPinsHit(...$pins) {
    return array_sum($pins) == 0 ? '0' : array_sum($pins);
}
?>
